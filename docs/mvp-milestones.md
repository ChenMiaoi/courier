...
2. 标题规范：自动将标题规范为单一 `Re: ...`（不重复前缀，保留 `[PATCH ...]` 标签）。
3. 收件人填充：`To/Cc` 继承原邮件并去重；若包含自己地址则自动移除。
4. 发件人填充：`From` 自动读取 git email 信息。
5. 线程头构造：自动填充 `In-Reply-To` 与 `References`。
6. 回信正文：按内核常见引用格式生成模板；在引用行按 `Enter` 自动续写 `> `。
7. Send Preview：发送前展示最终邮件预览（头部 + 正文），并校验必填项。
8. 预览确认门控：未确认预览时禁止发送动作。
9. 测试补齐：覆盖 `Re` 规范化、去自己、引用模板、预览渲染与校验失败路径。
...
3. `To/Cc` 继承原邮件并自动去自己。
4. `From` 自动来自 git email 配置。
5. `Enter` 在引用区可连续补 `> `，正文符合内核回信格式。
6. 可展示完整发送预览，且未确认时不能进入发送流程。
