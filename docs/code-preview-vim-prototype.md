...
本原型目标是在不离开 `Code Preview` 的前提下提供 Vim 风格编辑体验，并避免和现有全局键位冲突：

- 默认仍是浏览模式，沿用现有键位（`j/l` 焦点切换、`i/k` 移动）
...
- 未进入编辑模式时，`j/l/i/k`、`Tab`、命令栏、搜索、patch 操作行为不变
