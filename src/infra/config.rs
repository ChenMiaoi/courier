...
//! runtime shape. Paths, config aliases, and defaults are normalized here so
...
fn build_runtime_config(
    ...
    // instead of the process cwd so `criew --config <file>` stays portable.
...
fn select_or_initialize_default_config_path(home_dir: &Path) -> Result<(PathBuf, PathBuf)> {
    ...
    // Auto-initialize only after both supported locations are absent. That
    // keeps first-run setup zero-touch without overwriting user-managed files.
    ...
    F: Fn(&str) -> Option<String>,
{
    ...
    let config = ImapConfig {
        ...
        proxy: normalize_optional_string(file_config.proxy.clone()).or_else(|| {
            // Check CRIEW-specific proxy configuration first, then honor the
...
{
    // Prefer CRIEW config over git config so reply identity follows the mail
